...
describe("CLI", () => {
    ...
    it("should return non-zero status code when type-checking fails", () => {
        ...
        expect(isRunning(child.pid)).toBe(false);
        expect(child.stdout.toString()).toMatch(
            "'string' is not assignable to type 'number'"
        );
    });
    test.each(testCasesTs)(
        "should work if tsconfig is specified - $file",
        ({ file, statusCode }) => {
    ...
    test.each(testCasesJs)(
        ...
        ({ file, statusCode }) => {
            const child = cliSync`${getFixtureFile(file)} -p ${getFixtureFile(
                "tsconfig.json"
            )} --checkJs false --noEmit -p ${getFixtureFile(
                "tsconfig.json"
            )} --checkJs true`;
            expect(child.status).toBe(statusCode);
            if (statusCode !== 0) {
    ...
    test.each(testCasesTs)(
        "should work if excludeFiles flag is used - $file",
        ({ file, statusCode }) => {
    ...
    test.each(testCasesTs)(
        "should properly remove temp file - $file",
        ({ file }) => {
            const fileCountBeforeProcess = readdirSync(
                path.join(__dirname, "../fixtures")
            ).length;
            const child = cliSync`${getFixtureFile(file)} -p ${getFixtureFile(
                "tsconfig.json"
            )} --noEmit`;
            const fileCountAfterProcess = readdirSync(
                path.join(__dirname, "../fixtures")
            ).length;
